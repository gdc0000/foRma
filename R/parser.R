...
parse_and_prepare_data <- function(json_data) {
  ...
  if ("name" %in% names(nodes_df)) nodes_df <- rename(nodes_df, label = any_of("name"))
  ...
  # Sinonimi Edge
  if ("estimate" %in% names(edges_df)) edges_df <- rename(edges_df, label = any_of("estimate"))
  if ("weight" %in% names(edges_df)) edges_df <- rename(edges_df, label = any_of("weight"))
  ...
  if (!"lineType" %in% names(edges_df)) {
    edges_df <- edges_df %>%
      mutate(lineType = ifelse(.data$sourceId == .data$targetId, "curved", "straight"))
  }
  ...
  edges_augmented <- edges_df %>%
    ...
    rename(x_start_center = x, y_start_center = y, type.source = type, width.source = width, height.source = height) %>%
    left_join(nodes_df %>% select(any_of(node_cols)), by = c("targetId" = "id"), suffix = c(".source", ".target")) %>%
    rename(x_end_center = x, y_end_center = y, type.target = type, width.target = width, height.target = height)
